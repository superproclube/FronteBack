...
// Rota para buscar dados de esportes
app.get('/api/sports', async (req, res) => {
    try {
        ...
        // Processa os dados e retorna apenas as informações necessárias
        const processedData = data.response.map(match => ({
            league_name: match.league.name,
            league_logo: match.league.logo,
            home_team: match.teams.home.name,
            away_team: match.teams.away.name,
            match_time: match.fixture.date,
            probability: calculateProbability(match.goals.home, match.goals.away)
        }));

        res.json(processedData);
    } catch (error) {
        console.error('Erro na rota de esportes:', error);
        res.status(500).json({ error: 'Erro ao buscar dados de esportes' });
    }
});
